...
let makeVariAbstract ~varMap:mapping ~envCounter ~tempCounter
    ~position:currentPosition ~useMap:lastuses ~isQuery vari =
...
let rec makeTermAbstract ~varMap:mapping ~envCounter:counter ~useMap:lastuse
...
let makeBodyValAbstract ~varMap:mapping ~envCounter:counter ~useMap:lastuse
...
let getPositionLastVarUse (Clause (head, terms)) =
  ...
  let _ =
    updateLastVariableUsePositionAtom result 0 head
...
let rec genAbstCBVList ~varMap:mapping ~envCounter:envVars ~useMap:lastuse
    ~position:num ~tempCounter ~isQuery:q bvs =
  match bvs with
  | [] -> []
  | t :: ts ->
      let x =
...
let genFunctionTable (Program (Sentence prog, Resolvant goals)) =
  let cls =
    List.filter ~f:(fun x -> match x with C _ -> true | _ -> false) prog
  in
  ...
  let table =
    genFunctionTableH
      (List.map ~f:makeClauseAbstract cls)
      (Hashtbl.create functionTKeyImp)
  in
  let () = logDebug (fun m -> m "Finish with clauses body") in
  let mapping = Hashtbl.create strKeyImp in
  let lastuse = Hashtbl.create strKeyImp in
  let () = getTermsLastUse lastuse 0 goals in
  ...
  in
  let query =
    genAbstCBVList ~varMap:mapping ~envCounter:counter ~useMap:lastuse
      ~position:1 ~tempCounter ~isQuery:true goals
  in
  (table, (query, !counter))
