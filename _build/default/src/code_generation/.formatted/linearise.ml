...
let rec getInstructionNumClauseCodes clauseCodes =
  ...
  | [] -> 0
  | (_pos, clauseInstructions) :: cs ->
      List.length clauseInstructions + getInstructionNumClauseCodes cs

let rec getInstructionNumPredCodes predCodes =
  match predCodes with
  | [] -> 0
  | ((_id : functionId), xs) :: ys ->
      getInstructionNumClauseCodes xs + getInstructionNumPredCodes ys
...
let putIntoArray instructions positionT =
  let size = getInstructionNumPredCodes instructions in
  let codeArray = Array.create ~len:size Empty in
  let rec putInstructionsInArray pos instructions =
    ...
    | [] -> ()
  in
  let rec putClauseCodesInArray clauseCodes id (currentPosition : int) =
    match clauseCodes with
    | [] -> currentPosition
  ...
  in
  let rec putPredCodesInArray predCodes (currentPosition : int) =
    match predCodes with
    | [] -> ()
    | ((id : functionId), xs) :: ys ->
        (* Hashtbl.add_exn positionT ~key:(id,0) ~data:currentPosition; *)
...
let renameInstruction i positionT =
  match i with
  | Call (AbstractF fn) -> (
      let pos = Hashtbl.find positionT (AbstractF fn, 0) in
      match pos with
      | Some p -> Call (PositionF p)
      | None ->
          raise
            (Err
               ( "Attempt to call predicate that doesn't exist "
  ...
  | TryMeElse (AbstractC (AbstractF fn, num)) ->
      let pos = Hashtbl.find_exn positionT (AbstractF fn, num) in
      TryMeElse (PositionC pos)
  | RetryMeElse (AbstractC (AbstractF fn, num)) ->
      let pos = Hashtbl.find_exn positionT (AbstractF fn, num) in
      RetryMeElse (PositionC pos)
  | a -> a
...
let lineariseCode is =
  let positionT = Hashtbl.create clauseIdImp in
