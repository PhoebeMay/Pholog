...
let rec unifyTypesOfInferredVars result a b =
  let () = logDebug
      (fun m -> m "unifyTypesOfInferredVars %a and %a\n\n" pp_infv a pp_infv b)
  in
  match a, b with
...
(* unify a declared type variable with a renamed type variable *)
let rec unifyTypesDecVarToRenamed predVariableBindings bodyVariableBindings a b =
  let () = logDebug
      (fun m -> m "unifyTypesDecVarToRenamed %a  and %a\n\n"
...
let rec checkIsSubType predTypeMapping varTypeMapping
  ...
  let () = logDebug
      (fun m -> m "check is sub type list %a %a \n\n"
