...
(* Check two types are exactly equivalent *)
let rec checkTypeEquiv x y =
  ...
  | IntTyp, _ -> raise (Err "Int only equiv to int")

  | TypeCons(name,args),TypeCons(name2,args2) ->
    let () = logDebug (fun m -> m "check type equiv constructors :
          \n %a : %a  \n\n\n\n %a : %a\n"
                          pp_var name pp_t1 args pp_var name2 pp_t2 args2)
    in
    if name = name2 && List.length args = List.length args2
    then List.iter2_exn ~f:(checkTypeEquiv) args args2
    else raise (Err ("Type constructor "^(show_var name)
                     ^" does not unify with "^(show_var name2)))

  | TypeCons(_,_), _ -> raise (Err "")

type a = (funct * intnum) typeins list
[@@deriving show]
type b = var typeins list
[@@deriving show]

let rec checkTypesAreEquivalent (list1 : (var * intnum) typeins sexp_list)
    (list2 : var typeins sexp_list) =
  let () = logDebug
      (fun m -> m "check types are equivalent list %a %a \n\n"
  ...
  | [],[] -> ()
  | _ ->
    raise (Err "type equalence fails because type lists are different in length")
