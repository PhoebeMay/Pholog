...
let typeCheckClause clause predTypes typeDefs constructorToType =
  ...
  let () =
    Hashtbl.iteri
      ...
      predTypes
  in
  let predType = Hashtbl.find_exn predTypes name in
  let variableTypes =
  ...
  in
  let () =
    logDebug (fun m -> m "Arg type  list is %a" pp_argtypelist argTypeList)
  in
  let () = checkTypesAreEquivalent argTypeList predType in
  let () = logDebug (fun m -> m "Arg types are equivalent ") in
  let binding = Hashtbl.create strIntKeyImp in
  let () =
    List.iter
      ~f:(typeCheckCBV predTypes variableTypes constructorToType binding)
      body
  in
  let () = logBindings binding in
  ()
...
let checkPredTypesValid predTypes typeDefs =
  ...
  in
  Hashtbl.iteri
    ~f:(fun ~key:_ ~data:d -> List.iter ~f:checkTypeInsAppears d)
    predTypes

let typeCheck tc (Program (Sentence body, Resolvant goals)) =
  if not tc then logInfo (fun m -> m "Type checking disabled\n")
  else (
