...
type code =
  (functionId * (int * instruction list) list) list
  * Dt.arrLens
  * Dt.structureLookup
[@@deriving sexp]
...
let codeArgTerm (arg : location term) argnum seenSoFar nextFreeTemp
  ...
  | TFun (funct, ts) ->
      (*GET TS*)
      GetStructureA
        ((getStructMapInt structMapGen funct, List.length ts), Arg argnum)
...
let rec codeArgs ars counter seenSoFar nextFreeTemp structMapGen =
  ...
  | (a : location term) :: args ->
      let code1 = codeArgTerm a counter seenSoFar nextFreeTemp structMapGen in
      let code2 =
        codeArgs args (counter + 1) seenSoFar nextFreeTemp structMapGen
      in
      code1 @ code2
...
let codeBody (bodyVals : location clauseBodyVal list)
  ...
  | _ ->
      List.concat
        (List.map ~f:(codeBodyVal seenSoFar nextFreeTemp structMapGen) bodyVals)
...
let codeClause ((clause, n) : location clause * int) arrLens structMapGen =
  ...
  let maxTemp = getMaxTempClause clause in
  let nextFreeTemp = ref (maxTemp + 1) in
  let codeInsideClause (args : location term list) tail =
    let load = codeArgs args 0 seen nextFreeTemp structMapGen in
    let call = codeBody tail seen nextFreeTemp structMapGen in
    let argNum = List.length args in
    let tempNum = !nextFreeTemp in
    let () = arrLens.maxArg <- max argNum arrLens.maxArg in
    let () = arrLens.maxTemp <- max tempNum arrLens.maxTemp in
    load @ call
  in
  let result =
    match (clause, n) with
...
let rec codeNotFirstClauses id clauses counter arrLens structMapGen =
  ...
  | c :: cs ->
      let head =
        RetryMeElse (AbstractC (id, counter + 1))
...
let codeFirstClause id clauses arrLens structMapGen =
  ...
  | c :: cs ->
      ...
      in
      (0, head) :: codeNotFirstClauses id cs 1 arrLens structMapGen

let codePred id
    (functionTable :
      (functionId, (location clause * int) sexp_list) Base.Hashtbl.t) arrLens
    structMapGen =
  (* Used to have List.rev here *)
  let clauses = List.rev (Hashtbl.find_exn functionTable id) in
  (id, codeFirstClause id clauses arrLens structMapGen)

let codeAllClauses
    (functionTable :
      (functionId, (location clause * int) sexp_list) Base.Hashtbl.t) arrLens
    structMapGen =
  let fids = Hashtbl.keys functionTable in
  List.map ~f:(fun id -> codePred id functionTable arrLens structMapGen) fids
