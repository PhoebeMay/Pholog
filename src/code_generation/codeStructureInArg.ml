...
type abstractInstruction1 = Abs_UnifyE of location
...
(* Perform lookup in struct map,
   adding a new variable - int binding if there isnt one already *)
let getStructMapInt (structMapGen : structMapGen) str =
  let x = Hashtbl.find structMapGen.mapping str
  in match x with
  | Some(n) -> n
  | None ->
    let next = structMapGen.nextFree
    in
...
and codeArgStructsAbs terms (nextFreeTemp : int ref) structMapGen =
  ...
  in let collect results =
       let collectHead results  =
         List.map
           ~f:(fun {unify = h; body = _} -> h )
           results
       in let collectTail results  =
            List.map
              ~f:(fun {unify = _; body = t} -> t)
              results
...
let removeAbstractInstr xs seenSoFar = List.map
    ~f:(removeAbstractInstrFun seenSoFar) xs

let codeArgStructs xs nextFreeTemp seenSoFar structMapGen =
  removeAbstractInstr (codeArgStructsAbs xs nextFreeTemp structMapGen) seenSoFar
