...
let printUnifier hashT =
  ...
  Hashtbl.fold (fun key value _ -> Printf.printf "%s -> %s\n" (show_term pp_placeholder key) (show_term pp_placeholder value)) hashT ();
  print_endline ""
...
let strbtselem (a,b,c) =
  let (newc) = List.of_seq (Hashtbl.to_seq c)
  in
  (show_ab (a,b)) ^
  (show_c newc)
...
type env = E of placeholder atom list * (placeholder term, placeholder term) Hashtbl.t * env ref | Base
...
let prove functionT goals =
  ...
  in let rec proveAll goals clauses  s  =
       ...
       | (g::gs) ->
         ...
         try(
           let () = logDebug (fun m -> m "prove goal  %a" (pp_atom pp_placeholder) g) in
           (* get substitution for g *)
           let subst =  match clauses with
             | [] -> raise Backtrack
             | ((Clause(head, elems))::_) ->
               ...
               | Some(bottomSub, topSub) ->
                 ...
                 in let newS = concat topSub bottomSub
                 in match newGoalsInsideChosenClause with
                 |  [] ->
                   ...
                   newS
                 |  (x::_) ->  let id = genFunctionId x
                   in let nclauses = reverse (Hashtbl.find_all functionT id)

                   in let s = (proveAll newGoalsInsideChosenClause nclauses newS)
           ...
           let () = logDebug (fun m -> m " translated goal is  %a" (pp_atom pp_placeholder) (atomSubst subst g))
           in
           let remainingGoals = (map (fun atom -> atomSubst subst atom )) gs
           in
           (* let () = logDebug (fun m -> m "remainingGoals  %a" (pp_pal) remainingGoals) *)

           let newsub = concat subst s
           in
           match remainingGoals with
           |  [] -> newsub
           |  (x::_) ->  let id = genFunctionId x
             in let nclauses = reverse (Hashtbl.find_all functionT id)
             in (proveAll remainingGoals nclauses newsub)
  ...
  | (x::_) -> let id = genFunctionId x
    in let nclauses = reverse (Hashtbl.find_all functionT id)
    in try (Some(proveAll goals nclauses base))
    with Backtrack -> None
