...
let functor_test () =
  testLexerParser "inStorePair(X,Y):-inStore(pair(X,Y))."
    (Program(
        Sentence([
            Clause(Atom("inStorePair",[TVar("X");TVar("Y")]),[
                Atom("inStore",[
                    TFun("pair",[TVar("X");TVar("Y")])
                  ])
              ])]),
        Resolvant([])
      )
...
let multi_clause_test () =
  testLexerParser
    ...
    (Program(
        Sentence([
            Clause(Atom("fluffy",[TVar("X")]),
                   [Atom("cat",[TVar("X")])]);
            Clause(Atom("animal",[TVar("X")]),
                   [Atom("cat",[TFun("x",[])])])]),
        Resolvant([]))
    )
