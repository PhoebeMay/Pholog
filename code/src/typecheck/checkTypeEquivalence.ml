...
let rec checkTypeEquiv x y =
  let () = logDebug (fun m -> m "check type equiv h %a and %a" pp_in1 x pp_in2 y) in
  match x,y with
  | _,TypeVar(_) ->
...
let rec checkTypesAreEquivalent (list1 : (var * intnum) typeins sexp_list)
  ...
  let () = logDebug (fun m -> m "check types are equivalent list %a %a \n\n" pp_a list1 pp_b list2)
  in
  ...
  | [],[] -> ()
  | _ -> raise (Err "type equalence fails because type lists are different in length")
