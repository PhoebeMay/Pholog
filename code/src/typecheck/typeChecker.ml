...
let typeCheckClause clause predTypes typeDefs constructorToType =
  ...
  in let () = logDebug (fun m -> m "TYPE CHECK CLAUSE: typeCheckClause %a \n" pp_var name)
  in let () = Hashtbl.iteri
         ~f:(fun ~key:k ~data:_ -> (logDebug (fun m -> m "%a" pp_var k)))
         predTypes

  in let predType = Hashtbl.find_exn predTypes name
  in let variableTypes = getTypesOfVariables clause predTypes typeDefs constructorToType
  in let argTypeList = getPredicateArgumentTypes args variableTypes constructorToType

  in let () = logDebug (fun m -> m "Arg type  list is %a" pp_argtypelist argTypeList)
  ...
  in let () = List.iter ~f:(typeCheckCBV predTypes variableTypes constructorToType binding) body
...
let typeCheckQuery query predTypes  (constructorToType)=
  ...
  in let () = Hashtbl.iteri
         ~f:(fun ~key:k ~data:_ -> (logDebug (fun m -> m "%a" pp_var k)))
         predTypes
  in let variableTypes = getTypesOfVariablesQuery query predTypes constructorToType
  in let binding =  (Hashtbl.create strIntKeyImp)
  in let () = List.iter ~f:(typeCheckCBV predTypes variableTypes constructorToType binding) query
  in ()
  ...
  let checkTypeInsAppears x =
    ...
    | TypeCons(name,args) ->
      (match Hashtbl.find typeDefs name
       with Some(_) -> ()
          | None -> raise (Err ("Could not find definition for "^name^(show_intnum (List.length args)))))
    | TypeVar(_) -> ()
    | IntTyp -> ()
  in
  Hashtbl.iteri
