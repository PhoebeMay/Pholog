...
let rec unifyTypesOfInferredVars result a b =
  let () = logDebug (fun m -> m "unifyTypesOfInferredVars %a and %a\n\n" pp_infv a pp_infv b) in
  match a, b with
  ...
  | TypeCons(n1,ag1),TypeCons(n2,ag2)->
    if n1 = n2
    then List.iter2_exn ~f:(unifyTypesOfInferredVars result) ag1 ag2
...
let rec unifyTypesDecVarToRenamed predVariableBindings bodyVariableBindings a b =
  let () = logDebug (fun m -> m "unifyTypesDecVarToRenamed %a  and %a\n\n" (pp_typeins pp_var) a  pp_infv b)
  in
  match a, b with
  | TypeVar(x), y ->
  ...
  | TypeCons(n1,ag1),TypeCons(n2,ag2)->
    if n1 = n2
    then List.iter2_exn
        ~f:(unifyTypesDecVarToRenamed predVariableBindings bodyVariableBindings)
        ag1 ag2
    else raise (Err "types no unify")

  | TypeCons(_,_),TypeVar("Any",-1) -> ()

  | TypeCons(_,_),TypeVar(_,_) ->
    raise (Err "Case should not occur")
...
let unifyTypesList xs ys=
  ...
  in let () = List.iter2_exn ~f:(unifyTypesOfInferredVars result) xs ys
  in result


let unifyTypesDecVarToRenamedList xs ys=
  let predBindings = Hashtbl.create strKeyImp
  in let varBindings = Hashtbl.create strIntKeyImp
  in let () = List.iter2_exn ~f:(unifyTypesDecVarToRenamed predBindings varBindings) xs ys
  in (predBindings, varBindings)
...
let rec appSubstTypeinst subst typ = match typ with
  | TypeVar(x) -> let y = Hashtbl.find subst x
    in (match y with
        | Some(e) -> e
        | None -> TypeVar(x)
      )
  | IntTyp -> IntTyp
  | TypeCons(n,args) -> TypeCons(n, List.map ~f:(appSubstTypeinst subst) args)
...
let rec checkSubTypeBound varTypeMapping x y =
  ...
  | TypeVar(n1,0), TypeVar(n2,0) ->
    if n1 = n2
    then ()
    else raise (Err "No sub type user defined var")
  | TypeVar(x,0), TypeVar(y,n) ->
    (
  ...
  | TypeVar(y,n), TypeVar(x,n2) ->
    (
      ...
      then
        match Hashtbl.find varTypeMapping (y,n) with
        | None -> Hashtbl.add_exn varTypeMapping ~key:(y,n) ~data:(TypeVar(x,n2))
        | Some(res) -> checkSubTypeBound varTypeMapping res (TypeVar(x,n2))
      else
        match Hashtbl.find varTypeMapping (x,n2) with
        | None -> Hashtbl.add_exn varTypeMapping ~key:(x,n2) ~data:(TypeVar(y,n))
...
let rec checkSubType predTypeMapping varTypeMapping x y =
  ...
  let () = logDebug (fun m -> m "check is sub type %a %a \n\n" (pp_typeins pp_var) x (pp_typeins pp_strint) y)
  in
  ...
  | TypeVar(x),TypeVar(n,0) ->
    (match Hashtbl.find predTypeMapping x with
     | None ->
       Hashtbl.add_exn predTypeMapping ~key:x ~data:(TypeVar(n,0))
     | Some(e) ->
       if e = (TypeVar(n,0))
       then ()
       else
         logDebug (fun m -> m "found %a" pp_infv e);
  ...
  | TypeVar(x), TypeVar(name,num) ->
    (match Hashtbl.find predTypeMapping x with
     | None -> Hashtbl.add_exn predTypeMapping ~key:x ~data:(TypeVar(name,num))
     | Some(e) ->
  ...
  | _,TypeVar(_,0) -> raise (Err "Will not unify type constrained by annotation")
  ...
  | TypeCons(name,args),TypeCons(name2,args2) ->
    if name = name2
    then List.iter2_exn ~f:(checkSubType predTypeMapping varTypeMapping) args args2
    else raise (Err ("Type constructor "^(show_var name)^" does not unify with "^(show_var name2)))
  | TypeCons(_,_), _ -> raise (Err "")


let rec checkIsSubType predTypeMapping varTypeMapping (list1 : string typeins sexp_list) (list2 : (string * intnum) typeins sexp_list) =
  let () = logDebug (fun m -> m "check is sub type list %a %a \n\n" pp_typlis list1 pp_typlisn list2)
  in
  match list1,list2 with
  | x::xs,y::ys -> checkSubType predTypeMapping varTypeMapping x y;
