...
module DriverAndFunctions (State : StateTyp)  : DriverAndFunctionsTyp = struct
  ...
  module Driver  (I : InstructionFunctionsTyp)  : DriverTyp = struct
    ...
    let rec execute compState : 'a option =
      logDebug (fun m -> m "%a . %a" pp_intnum (compState.cp) pp_instruction (Array.get State.instructions compState.cp));
      logDebug (fun m -> m "CP stack length: %a" pp_intnum (getLength compState.choicePoints));
      ...
      | RetryMeElse(PositionC(n)), Some(c), _ ->
        ...
        let () = updateChoicePoint n c.value
        in
      ...
      | AddI(Temp(t),n),_,_ ->
        ...
        in (match oldVal with
            | TempInt(x) ->
              let () = Array.set compState.temps t (TempInt(n + x)); compState.cp <- compState.cp + 1
              in execute compState
      ...
      | SubI(Temp(t),n),_,_ ->
        ...
        in (match oldVal with
            | TempInt(x) ->
              let () = Array.set compState.temps t (TempInt(x - n)); compState.cp <- compState.cp + 1
              in execute compState
            | _ -> raise (Err "Expected accumilator to be an int")
          )
      ...
      | RmCps, _,Some(e) ->
        ...
        let _ =
          if List.exists (getPreds (e.value.callerCps))
              ~f:(fun x -> Some(x) = compState.choicePoints)
...
(* in let _ = raise Oops *)
      ...
      | Finish, _, Some(e) ->
        (match !State.resCount with
         ...
         | n -> State.resCount := n-1;
           ...
           in let flat =  List.map ~f:(flattenSv State.structMap) ( Array.to_list thisans)
  ...
  and InstructionImplementationsImpl : InstructionFunctionsTyp = InstructionImplementations (DriverImpl)
end
