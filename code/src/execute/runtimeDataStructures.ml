...
module type InstructionFunctionsTyp =
sig
  val performArith : compState ->
    location -> intnum -> environment node -> (intnum -> intnum -> intnum) -> environment option

  val evalIs : variable ->
    intnum -> variable sexp_array -> intnum -> compState -> environment option

  val getinta  : variable -> int -> compState -> environment option


  val getStructure : int ->
    int -> variable array -> int -> compState -> environment option


  val putStructure : intnum ->
    intnum -> variable sexp_array -> intnum -> compState -> environment option


  val unifyInFromHeap : variable sexp_array -> intnum -> compState -> environment option

  val setValue : variable sexp_array -> intnum -> compState -> environment option
