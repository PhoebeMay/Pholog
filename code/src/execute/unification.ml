...
let rewindStack (c:choicePoint node) compState=
  c.value.stack
...
let backtrack compState =
  ...
  | None -> raise Fail
  | Some(c) ->
    let newTrail = rewindTrail compState.trail
...
let rec unifyLists xs  ys compState =
  ...
  let rec unifyListsH xs ys counter compState =
    ...
    then (
      ...
      let cp = compState.cp
      in let newCompState =
        unifyH (Array.get xs counter) (Array.get ys counter) compState
      in
      if newCompState.cp = cp
      then
        unifyListsH xs ys (counter + 1) newCompState
      else  compState

  in unifyListsH xs  ys 0 compState

and unifyH ah bh compState  =
  let () = logDebug (fun m -> m "unifyH %a %a" pp_heapValue ah pp_heapValue bh) in
  ...
  | Int(n1),Int(n2) ->
    ...
    else backtrack compState
  | x, y ->

    logError (fun m -> m "UnifyH failed: %a %a \n" pp_heapValue x pp_heapValue y );
...
and tryBind x y compState=
  ...
  then (
    if not(cyclic (B(x)) y)
    then (
      let newTrail = createNew (Var(x)) ~prev:compState.trail
      in   (x) :=  y;
      compState.trail <- newTrail;
      compState
    )
...
let unifyS a b compState =
  ...
  | _, InitV -> raise Oops
  | HeapPointer(ah), HeapPointer(bh) ->
    unifyH ah bh compState
  | _, _ -> raise Oops
