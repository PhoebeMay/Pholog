...
let renameInstruction i positionT=
  ...
  | Call(AbstractF(fn)) ->
    ...
    | None -> raise (Err ("Attempt to call predicate that doesn't exist " ^ (show_functionId (AbstractF(fn))) ) )
      )
...
let renameArray positionT instructionArray =
  Array.map_inplace ~f:(fun i -> renameInstruction i positionT) instructionArray;
  instructionArray
