...
(*
let setIfNotAlready hashT key counter =
  try (
    Hashtbl.find_exn hashT key
  )
  with Caml.Not_found
    ->
    Hashtbl.add_exn hashT ~key:key ~data:(!counter);
    counter := !counter + 1;
    !counter - 1 *)

let makeVariAbstract ~varMap:mapping ~envCounter:envCounter ~tempCounter:tempCounter ~position:currentPosition ~useMap:lastuses ~isQuery:isQuery vari =
...
let rec makeTermAbstract ~varMap:mapping ~envCounter:counter ~useMap:lastuse ~position:currentPosition ~tempCounter:tempCounter ~isQuery:q term =
  match term with
  | TVar(var) -> TVar(makeVariAbstract ~varMap:mapping ~envCounter:counter ~useMap:lastuse ~position:currentPosition ~tempCounter:tempCounter ~isQuery:q var)
  | TFun(funtor, ts) ->
    TFun(funtor, List.map ~f:(makeTermAbstract ~varMap:mapping ~envCounter:counter ~useMap:lastuse ~position:currentPosition ~tempCounter:tempCounter ~isQuery:q) ts)
  | TInt(n) -> TInt(n)

let rec makeMathExprAbstract ~varMap:mapping ~envCounter:counter ~useMap:lastuse ~position:currentPosition ~tempCounter:tempCounter ~isQuery:q expr =
  match expr with
  | Subtract(e1,e2) ->
    Subtract(
      makeMathExprAbstract ~varMap:mapping ~envCounter:counter ~useMap:lastuse ~position:currentPosition ~tempCounter:tempCounter ~isQuery:q e1,
      makeMathExprAbstract ~varMap:mapping ~envCounter:counter ~useMap:lastuse ~position:currentPosition ~tempCounter:tempCounter ~isQuery:q e2)
  | Plus(e1,e2) ->
    Plus(
      makeMathExprAbstract ~varMap:mapping ~envCounter:counter ~useMap:lastuse ~position:currentPosition ~tempCounter:tempCounter ~isQuery:q e1,
      makeMathExprAbstract ~varMap:mapping ~envCounter:counter ~useMap:lastuse ~position:currentPosition ~tempCounter:tempCounter ~isQuery:q e2)
  | Base(Int(n)) -> Base(Int(n))
  | Base(Var(v)) -> Base(Var(makeVariAbstract ~varMap:mapping ~useMap:lastuse ~position:currentPosition ~envCounter:counter ~tempCounter:tempCounter ~isQuery:q v))


(*
let makeAtomAbstract mapping counter (Atom(name,args)) =
  Atom(name, map (makeTermAbstract mapping counter) args) *)

let makeBodyValAbstract ~varMap:mapping ~envCounter:counter ~useMap:lastuse ~position:currentPosition ~tempCounter:tempCounter ~isQuery:q item =
  match item with
  | CAT(Atom(name,args)) ->
    CAT(Atom(name,
             List.map
               ~f:(makeTermAbstract ~varMap:mapping ~envCounter:counter ~useMap:lastuse ~position:currentPosition ~tempCounter:tempCounter ~isQuery:q)
               args
            ))
  | CAR(IsExpr(vari, mathexpr)) ->
    CAR(IsExpr(
        makeVariAbstract ~varMap:mapping ~envCounter:counter ~useMap:lastuse ~position:currentPosition ~tempCounter:tempCounter ~isQuery:q vari,
        makeMathExprAbstract ~varMap:mapping ~envCounter:counter ~useMap:lastuse ~position:currentPosition ~tempCounter:tempCounter ~isQuery:q mathexpr
      ))
  | Cut -> Cut
  | Fail -> Fail
...
let rec updateLastVariableUsePositionMathExpr result currentPosition mathexpr =
  match mathexpr with
  |   Plus(x,y) -> updateLastVariableUsePositionMathExpr result currentPosition x;
    updateLastVariableUsePositionMathExpr result currentPosition y
  | Subtract(x,y) -> updateLastVariableUsePositionMathExpr result currentPosition x;
    updateLastVariableUsePositionMathExpr result currentPosition y
  | Base(Int(_)) -> ()
  | Base(Var(var)) -> let x = Hashtbl.change result var ~f:(fun _ -> Some(currentPosition))
    in x
...
let rec updateLastVariableUsePositionClauseBodyVal result currentPosition bv  =
  ...
  match bv with
  | CAT(Atom(name,args)) -> List.iter args ~f:(updateLastVariableUsePositionTerm result currentPosition)
  | CAR(IsExpr(vari, mathexpr)) -> updateLastVariableUsePositionMathExpr result currentPosition mathexpr;
...
let getPositionLastVarUse (Clause(head, terms)) =
  ...
  (* in let rec applyToTerms counter x =
       match x with
       | [] -> ()
       | (t::ts) ->
         logDebug (fun m -> m "hi");
...
let rec genAbstCBVList ~varMap:mapping ~envCounter:envVars ~useMap:lastuse ~position:num ~tempCounter:tempCounter ~isQuery:q bvs =
  match bvs with
  | [] -> []
  | t::ts -> let x = makeBodyValAbstract ~varMap:mapping ~envCounter:envVars ~useMap:lastuse ~position:num ~tempCounter:tempCounter ~isQuery:q t
    in x::(genAbstCBVList ts ~position:(num+1) ~varMap:mapping ~envCounter:envVars ~useMap:lastuse ~tempCounter:tempCounter ~isQuery:q)
...
let makeClauseAbstract c =
  ...
  | C((Clause(Atom(name, args), terms))) ->
    ...
    in let abstHead =   List.map ~f:(makeTermAbstract ~varMap:mapping ~envCounter:envVars ~useMap:lastuse ~position:0 ~tempCounter:tempCounter ~isQuery:false) args
    ...
    in let abstTail = genAbstCBVList ~varMap:mapping ~envCounter:envVars ~useMap:lastuse  ~position:1 ~tempCounter:tempCounter ~isQuery:false terms
...
let genFunctionTable ((Program(Sentence(prog),Resolvant(goals))) ) =
  let cls = List.filter ~f:(fun x -> match x with C(_) -> true | _ -> false ) prog
  in let rec genFunctionTableH clauses (hashT)  =
       match clauses with
       | [] -> hashT
       | ((Clause(Atom(name,args), body),n)::cs) ->
         Hashtbl.add_multi hashT ~key:(AbstractF(name, List.length args)) ~data:(Clause(Atom(name,args), body),n);
