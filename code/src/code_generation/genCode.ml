...
type code = (functionId * (int * instruction list) list) list * Dt.arrLens * Dt.structureLookup [@@deriving sexp]
type functTlist = (functionId * functTval) list [@@deriving sexp ]



let codeArgTerm (arg : location term) argnum seenSoFar nextFreeTemp structMapGen=
  match arg with
  ...
  | TFun(funct, ts) ->
    GetStructureA((getStructMapInt structMapGen funct, List.length ts), Arg(argnum)) (*GET TS*)
...
let rec codeArgs ars counter seenSoFar nextFreeTemp structMapGen =
  ...
  | ((a:location term)::args) ->
    ...
    in let code2 =  codeArgs args (counter + 1) seenSoFar nextFreeTemp structMapGen
    in code1 @ code2
...
let loadArg argnum seenSoFar nextFreeTemp structMapGen (term : location term) =
  ...
  | TFun(funct, ts) ->
    ...
    in
    build @ [PutStructureA((getStructMapInt structMapGen funct, List.length ts), Arg(argnum)) ] @ load

let loadArgs (terms : location term list) (seenSoFar : location Hash_set.t) nextFreeTemp structMapGen =
  let argnum = ref 0
  in
  let ans  = List.concat (List.map ~f:(fun t -> argnum := !argnum + 1; loadArg (!argnum - 1) seenSoFar nextFreeTemp structMapGen t) terms);
  in ans
...
let codeBodyVal (seenSoFar : location Hash_set.t) nextFreeTemp structMapGen clauseBodyItem=
  ...
  | CAT(Atom(pred, terms)) ->   (loadArgs terms seenSoFar nextFreeTemp structMapGen) @ [Call(AbstractF(pred, List.length terms))]
  | Cut -> [RmCps]
  | Fail -> [Backtrack]
  | CAR(IsExpr(var,body)) ->
...
let codeBody (bodyVals : location clauseBodyVal list) (seenSoFar : location Hash_set.t) nextFreeTemp structMapGen=
  match bodyVals with
  | [] -> []
...
let rec getMaxTempTerm result term =
  match term with
  | TVar(T(Temp(pos))) -> if pos > !result then result := pos else ()
  | TFun(_, args) -> List.iter ~f:(getMaxTempTerm result) args
  | _ -> ()
...
let rec getMaxTempMathExpr result mathExpr =
  match mathExpr with
  | Plus(x,y) -> getMaxTempMathExpr result x; getMaxTempMathExpr result y
  | Subtract(x,y) -> getMaxTempMathExpr result x; getMaxTempMathExpr result y
  | Base(Int(_)) -> ()
...
let getMaxTempCBV result cbv =
  ...
  | CAR(IsExpr(vari,mathexpr)) -> getMaxTempMathExpr result mathexpr;
    match vari with
...
and addTempClears defined instructions =
  ...
  | i::is -> (
      ...
      | StructGetVariable(T(t)) ->
        ...
        else (  Hash_set.add defined (T(t)); i::(addTempClears defined is))
...
let rec codeNotFirstClauses id clauses counter arrLens structMapGen=
  ...
  | [c] -> let r = [(counter,TrustMe :: codeClause c arrLens structMapGen )]
    in r
  | c::cs ->
    let head = (RetryMeElse(AbstractC(id,counter+1)) :: codeClause c arrLens structMapGen)
    in (counter,head) ::codeNotFirstClauses id cs (counter + 1) arrLens structMapGen
...
let codeFirstClause id (clauses) arrLens structMapGen=
  ...
  | c::cs ->
    let head = (TryMeElse(AbstractC(id,1)) :: codeClause c arrLens structMapGen)
    in (0,head) :: (codeNotFirstClauses id cs 1 arrLens structMapGen)

let codePred id (functionTable : (functionId, (location clause * int) sexp_list) Base.Hashtbl.t) arrLens structMapGen=
  (* Used to have List.rev here *)
  let clauses = List.rev (Hashtbl.find_exn functionTable id)
  in (id, codeFirstClause id clauses arrLens structMapGen)

let codeAllClauses (functionTable : (functionId, (location clause * int) sexp_list) Base.Hashtbl.t) arrLens structMapGen=
...
let codeQuery query arrLens structMapGen=
  ...
  in match query with (body,n)->
    let res = [(AbstractF("query",0),[ (0,Allocate n :: codeBody body seen nextFreeTemp structMapGen @ [Finish])])]
