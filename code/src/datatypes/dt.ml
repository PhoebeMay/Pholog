...
let intImp =
  (module struct
    ...
    let hash x = x
  end : Base__.Hash_set_intf.Key with type t = int )

type structureLookup =
  string Base.Hashtbl.M(Int).t
[@@deriving sexp]


type varLookup =
  var Base.Hashtbl.M(Int).t
[@@deriving sexp]


type structMapGen = {mapping : (string,int) Base.Hashtbl.t; mutable nextFree : int}

type functionId = AbstractF of (string * int) | PositionF of int [@@deriving sexp, show]


(* type row = functionId * clauseAndNum  [@@deriving show] *)
type clausePos = AbstractC of functionId * int | PositionC of int [@@deriving sexp, show]
...
let envImp =
  (module struct
    ...
    let hash (Env(x)) = x
  end : Base__.Hash_set_intf.Key with type t = env)
...
let locationImp =
  (module struct
    ...
    let hash x = match x with
      | E(Env(n)) -> n * 2
      | T(Temp(n)) -> (n * 2) + 1
  end : Base__.Hash_set_intf.Key with type t = location)
...
let strKeyImp =
  ...
  end :  Base__.Hashtbl_intf.Key with type t = string)
...
let strIntKeyImp =
  (module struct
    ...
    let hash (x,y) =
      let hash = Hash.create ~seed:(String.hash x ) ()
      in let hash2 = Hash.fold_int hash y
      in Hash.get_hash_value hash2
  end :  Base__.Hashtbl_intf.Key with type t = string*int)
(*
let fidKeyImp =
  (module struct
    type t = functionId
...
type instruction =
  ...
  | TrustMe
  | GetVariable of location * arg (*Variable not yet seen*)
  ...
  | ClearTemp of temp
  | Finish
  | Empty     (*Used to populate initially *)
  | RmCps
  | Backtrack
[@@deriving show, sexp]
...
type writtenInstr = {nums : arrLens; code: instruction sexp_array; structMap : structureLookup} [@@deriving sexp]



type heapValue = B of heapValue ref
               | InitH
               | UnboundVar
               | StrPointer of int * heapValue array
               | Int of int  [@@deriving show, sexp]
(* and heapVar = {mutable v : heapValue} [@@deriving show] *)


type variable = InitV | HeapPointer of heapValue | TempInt of int [@@deriving show, sexp ]



type heapValueFlat = | UnboundVarF | InitHF | StrPointerF of string * heapValueFlat array | IntF of int [@@deriving show]


type variableFlat = InitVF | HeapPointerF of heapValueFlat | TempIntF of int [@@deriving show]
