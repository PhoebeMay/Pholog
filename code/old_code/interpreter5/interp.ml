...
let concat out inn =
  let () =
    ...
    printUnifier out;
    ()
  in let inner = Hashtbl.copy inn
  in let outer = Hashtbl.copy out
  in
...
let prove functionT goals =
  ...
  let rec backTrack bts =
    print_endline "backtrack!";
    match bts with
    | ((goal,clauses,subst)::bs) ->(
        print_endline "backtrack stack is not empty :)";
        print_endline "it contains";
        print_endline (String.concat " "(map (fun clause -> strClause (genStringClause clause)) clauses));
        print_endline "subst on backtrack is";
        printUnifier subst;
        proveGoal goal clauses subst bs
      )

    | [] ->
      print_endline "backtrack stack is empty :(";
      None

  and proveGoal goal clauses s bts  =
    match clauses with
    | [] ->
      print_endline "Clauses empty";
      backTrack bts
    (* None *)
    ...
    | ((Clause(head, elems))::cs) ->
      ...
      in let () =
           print_endline "Backtrack clauses are";
           print_endline (String.concat " "(map (fun clause -> strClause (genStringClause clause)) cs))

      in let other = (goal, cs, s)

      in let newBackTrackStack = other::bts

      in match unifyAtom goal head with
      | None ->
        print_endline "unify fail goal clause";
      ...
      | Some (bottomSub, topSub) -> (
          ...
          in let () = print_endline "new B is";
               printUnifier newB

          in let () = print_endline "prove call 2"
  ...
  and proveAll goals st bts =
    ...
    | (g::gs) ->
      ...
      in let () =
           print_endline "Proveall";
           print_endline "Clauses:";
           print_endline (String.concat " "(map (fun clause -> strClause (genStringClause clause)) clauses));
           print_endline "Given subst:";
           printUnifier st


      (* in let subst = proveGoal g clauses st bts *)
...
(*
      ...
      | Some(substNewGoalsQ, _btsNew) ->
        ...
        in
        let newsub = concat substNewGoalsQ st
        in print_endline "concat 2";
        print_endline "prove call 1";
        match proveAll remainingGoals newsub bts
        with | None ->
          (let newTry = backTrack _btsNew in match newTry with
            | None -> ..
            | Some(s) -> Some(s)          )
