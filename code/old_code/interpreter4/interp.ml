...
let rec prove currentConsideredClauses functionT goals s bts =
  count := !count + 1;
  match goals with
  |  [] -> raise Feck
  | (g::gs) ->
    ...
    | ((Clause(head, elems))::cs) ->
      ...
      in let other = (fun () -> (prove cs functionT goals s bts))

      in let newBackTrackStack = other::bts

      in match unifyAtom g head with
      ...
      | Some (bottomSub, topSub) -> (
          ...
          in let substNewGoalsQ =  match newGoalsInsideChosenClause with
              | [] -> Some(newB)
              | (x::_) -> (
                  let id = genFunctionId x
                  in let clauses = reverse (Hashtbl.find_all functionT id)
                  in let ans = prove clauses functionT newGoalsInsideChosenClause newB []
                  in ans)

          in match substNewGoalsQ with
          | None -> backTrack newBackTrackStack
          | Some(substProveNewGoals) ->
            ...
            | [] ->
              ...
              Some(concat substProveNewGoals s)
            (* substProveNewGoals *)
            | (x::_) ->

              let id = print_endline "march rg 2";
                genFunctionId x
              in let clauses = reverse (Hashtbl.find_all functionT id)
              in let ans = prove clauses functionT remainingGoals (concat substProveNewGoals s) newBackTrackStack
              in ans
        )
